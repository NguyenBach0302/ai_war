...
export class MatchService {
    ...
    getServerDamage(sim: AnyObject, attacker: AnyObject, target: AnyObject, baseDmg: number, type: string): number {
    ...
    tryServerSkill(match: AnyObject, unit: AnyObject, target: AnyObject): boolean {
        ...
        if (lower.includes('iceman') && unit.mana >= icemanSkillCost) {
            ...
            const targets = this.queryServerSpatialUnits(sim, unit.x, unit.y, 200, (other: AnyObject) => other.owner !== unit.owner && other.hp > 0)
        ...
        if (lower.includes('healer') && unit.mana >= healerSkillCost) {
            const allies = this.queryServerSpatialUnits(sim, unit.x, unit.y, 200, (other: AnyObject) => other.owner === unit.owner && other.id !== unit.id && other.hp > 0 && other.hp < other.maxHp)
                .map((other: AnyObject) => ({ unit: other, distance: this.serverDist(unit, other) }));
            allies.sort((a, b) => a.distance - b.distance);
            const selectedAllies = allies.slice(0, 3).map(entry => entry.unit);
            if (!selectedAllies.length) return false;
            unit.mana -= healerSkillCost;
            unit.skillCooldown = 240;
            this.setServerAnim(unit, 'attack_3', sim.frame);
            selectedAllies.forEach((ally: AnyObject) => {
    ...
    updateServerSim(match: AnyObject): void {
        ...
        sim.units.forEach((unit: AnyObject) => {
            ...
            if (distance > range && (lowerType.includes('gunman') || lowerType.includes('gunner')) && distance <= range + 100 && this.tryServerSkill(match, unit, target)) return;
