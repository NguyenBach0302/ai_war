const Game = (function() {
    ...
    function updatePlayerHud(playerIndex, player) {
        const cached = playerHudCache[playerIndex] || {};
        ...
        if (cached.hpWidth !== hpWidth) {
            const hpEl = document.getElementById(`hp-${playerIndex}`);
            if (hpEl) hpEl.style.width = hpWidth;
            cached.hpWidth = hpWidth;
        }
        playerHudCache[playerIndex] = cached;
    }
    ...
    function update() {
        ...
        if (frameCount - lastUnitButtonsUpdateFrame >= 12) {
            updateUnitButtons();
            lastUnitButtonsUpdateFrame = frameCount;
        }
            ...
            if (d < 10) {
                ...
                units.forEach(eu => { 
                    if (eu.owner !== pr.owner && eu.untargetableTimer <= 0 && dist(pr, eu) < 25) { 
                        ...
                        if (dmgRes.dodged) spawnVFX(eu.x, eu.y, 'MISS', '#94a3b8');
                            ...
                            spawnVFX(eu.x, eu.y, (dmgRes.isCrit ? 'ðŸ’¥' : '') + Math.floor(dmgRes.amount), dmgRes.isCrit ? '#ff0000' : '#fff'); 
                ...
                players.forEach(p => {
                    if (!p.eliminated && p.id !== pr.owner && dist(pr, p.base) <= p.base.r + 15) {
                        ...
                        spawnVFX(pr.tx, pr.ty, (dmgRes.isCrit ? 'ðŸ’¥' : '') + Math.floor(dmgRes.amount), dmgRes.isCrit ? '#ff0000' : '#fff');
                    }
                });
                projectiles.splice(i, 1);
                ...
                if (d <= effectiveRange) {
                    if (u.cooldown <= 0) {
                        ...
                        if (isMeleeStrike) {
                            ...
                            else { target.hp -= res.amount; addParticle(tx, ty, res.isCrit ? '#ef4444' : players[u.owner].color.main, res.isCrit ? 14 : 7, res.isCrit ? 3 : 1.8); if (ls > 0) { const h = res.amount * ls; u.hp = Math.min(getEffectiveMaxHp(u), u.hp + h); spawnVFX(u.x, u.y, `+${Math.floor(h)}`, '#22c55e'); } spawnVFX(tx, ty, (res.isCrit?'ðŸ’¥':'')+Math.floor(res.amount), res.isCrit?'#ff0000':'#fff'); }
                        } else {
    ...
    function drawMapBackground() {
        if (!mapBackgroundCanvas || mapBackgroundCanvas.width !== MAP_W || mapBackgroundCanvas.height !== MAP_H) {
            mapBackgroundCanvas = document.createElement('canvas');
            mapBackgroundCanvas.width = MAP_W;
